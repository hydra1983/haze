...
public class User {
	...
	public void SetW(int pt,int pp,int pa){
		P=pt+pp+pa;
		this.Wt=pt/P;
		this.Wp=pp/P;
		this.Wa=pa/P;
	...
	*/
			...
			if(lev<min[t]){
				...
				if(lev==min[t]){
					...
					if(lev>=objT[t]){
						...
						return 1+4*(lev-min[t])/(objT[t]-min[t]);}
				}
			}
		}
	}
	...
	public void curArray(){
		...
		for(int i=0;i<5;i++){
			Umol=cursys.get(i);
			if(Umol!=null){
				if(Umol.StatementforU==false||Umol.Statement==0){
					cur[i]=0;}
			}
			else{
				cur[i]=0;}
		...
		for(int i=0;i<5;i++){
			if(cur[i]<min[i]){
				...
				ch=true;
					...
					Umol=cursys.get(i);
					if(Umol!=null){
						Vt=SetVt(i,exchT(Umol.Type));
				        Vp=SetVp(Umol.Price);
				        Va=SetVa(Umol.AgeforU);
				        Vi=SetVi(Vt,Vp,Va);
				        if(Vi<1){//change -1 to 2!!
				        	rep[i]=1;
				        	ch=true;
				        	}
				        //find that cur>min but gap<Vi?
				        else{
				    	    rep[i]=0;}
					}
					else{
